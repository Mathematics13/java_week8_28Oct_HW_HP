package package19_cylinder;

//Class 1 Circle
class Circle
{
   double radius;                                                            //instance variable of type double
    Circle(double radius)                                                  //Constructor with parameter radius of type double
    {

        if(radius<0)                                                    //In case the radius parameter <0 ,need to set the radius field value to 0
        {
            this.radius=0;
        }
        else
        {
            this.radius=radius;                                     //To initialize the fields
        }
    }
    public  double getRadius()                                     //get the value means return the value
    {
       return radius;
    }
    public  double getArea()                                   //get the area so return the area with its particular data type
    {
        double area = Math.PI*radius*radius;                  //area of the circle pi*r*r
        return area;
    }
}

/** Cylinder (Create package name ‘cylinder’ and create all below classes in this package)
 1. Write a class with the name Circle. The class needs one field (instance variable) with name radius of type double.
 The class needs to have one constructor with parameter radius of type double and it needs to initialize the fields.
 In case the radius parameter is less than 0 it needs to set the radius field value to
 Write the following methods (instance methods):
 ● Method named getRadius without any parameters, it needs to return the value of the radius field.
 ● Method named getArea without any parameters, it needs to return the calculated area (radius * radius * PI). For PI use Math.PI constant.
 */
